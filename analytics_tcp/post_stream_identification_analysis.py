...
if __name__ == "__main__":
    ...
    captures_meta = []
    type_counts = defaultdict(list[str])
    packet_pattern = re.compile(r"([\w-]+)_(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})_(\d{4})")
    ...
    for idx, capture in enumerate(captures):
        ...
        if meta:
            captures_meta.append({
                "capture": capture,
                "identifier": meta.group(1),
                "ts": f"{meta.group(3)}/{meta.group(4)} {meta.group(5)}:{meta.group(6)}"
            })
            type_counts[meta.group(1)].append(idx)
        ...
        for k, v in type_counts.items():
            ...
            if len(v) > 1:
                ...
                for v_ins in v:
                    ...
                    rtt_avgs.append((list(df['rtt_avg']), captures_meta[v_ins]['ts']))
                    s2c_rtt_avgs.append((list(df['dst2src_rtt_avg']), captures_meta[v_ins]['ts']))
                    c2s_rtt_avgs.append((list(df['src2dst_rtt_avg']), captures_meta[v_ins]['ts']))

                    win_avgs.append((list(df['window_avg']), captures_meta[v_ins]['ts']))
                    s2c_win_avgs.append((list(df['dst2src_window_avg']), captures_meta[v_ins]['ts']))
                    c2s_win_avgs.append((list(df['src2dst_window_avg']), captures_meta[v_ins]['ts']))

                    retrans.append((list(df['num_retransmission']), captures_meta[v_ins]['ts']))
                    outoforders.append((list(df['num_outoforder']), captures_meta[v_ins]['ts']))
                    dupacks.append((list(df['num_duplicate_ack']), captures_meta[v_ins]['ts']))

                    hdsk = list(df[df['handshake_duration'] > 0.0]['handshake_duration'])
                    if len(hdsk) > 0:
                        hdsk_avgs.append((hdsk, captures_meta[v_ins]['ts']))

                    tls_hdsk = list(df[df['tls_handshake_duration'] > 0.0]['tls_handshake_duration'])
                    if len(tls_hdsk) > 0:
                        tls_hdsk_avgs.append((tls_hdsk, captures_meta[v_ins]['ts']))
